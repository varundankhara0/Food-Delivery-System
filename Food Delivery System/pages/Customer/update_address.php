...
<body>
    ...
    <div class="form-section">
        <?php include "../../connection.php";
            $query="select * from Tbl_Customer_Address where id=".$id." and status=1";
            $result=mysqli_query($conn,$query);
            if($result->num_rows<=0)
            {
                echo "<script>alert('this paticular address does not exists or has been deleted by You please try again later')</script>";
            }
            while($row=$result->fetch_assoc())
            {
            ?>
        <h1>Update Customer's Address Registration</h1>
        <form method="post" action="#" enctype="multipart/form-data" id="user-form" novalidate>
            <div class="form-group">
                <input type="number" name="id" value="<?php echo $id?>" hidden>
                <label for="flatno">Flat No.:</label>
                <input type="text" name="flatno" id="flatno" placeholder="Enter Your Flat No" value="<?php echo $row["doorno"]?>" required>
                <div id="flatno-error" class="error">Please enter your flat number.</div>
            </div>
            <div class="form-group">
                <label for="address">Address:</label>
                <textarea name="address" id="address" placeholder="Enter Your Address" required><?php echo $row["address"]?></textarea>
    ...
    <script>
        $(document).ready(function() {
            $('#user-form').on('submit', function(e) {
                ...
                if (valid) {
                    $.ajax({
                        url: '../Ajax_files/Update_Address.php',
                        method: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
